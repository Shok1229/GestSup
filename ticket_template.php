...
if($_POST['duplicate'])
{
	...
	$query= "INSERT INTO tincidents (user,title,description,priority,state,time,category,subcat,date_create,technician,criticality,creator,type) VALUES ('$row[user]','$row[title]','$row[description]','$row[priority]','$row[state]','$row[time]','$row[category]','$row[subcat]','$datetime','$_SESSION[user_id]','$row[criticality]','$_SESSION[user_id]','$row[type]')";
