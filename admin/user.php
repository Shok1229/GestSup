...
if($_POST['Modifier'])
{
	...
	if($_POST['attachment'])
	{
	  	$query = 'INSERT INTO tusers_tech (user,tech) VALUES ('.$_POST['attachment'].','.$_GET['id'].')';
...
if (($_GET['action']=='edit') && (($_SESSION['user_id']==$_GET['id']) || ($_SESSION['profile_id']==0 || $_SESSION['profile_id']==4))) 
{
	//get user data
                            		...
                            		<div class="tabbable">
                            			<ul class="nav nav-tabs" id="myTab">';
                            				if($_GET['tab']=='infos') {echo '<li class="active" >';} else {echo '<li>';} echo '
                            					<a href="./index.php?page=admin/user&subpage=user&action=edit&id='.$_GET['id'].'&tab=infos">
                            						<i class="icon-info bigger-110 blue"></i>
                            						Informations
                            					</a>
                            				</li>';
                            				if($_GET['tab']=='parameters') {echo '<li class="active" >';} else {echo '<li>';} echo '
                            					<a href="./index.php?page=admin/user&subpage=user&action=edit&id='.$_GET['id'].'&tab=parameters">
                            				...
                            				if ((($user1['profile']==0) || ($user1['profile']==4)) && ($_SESSION['profile_id']!=1) && ($_SESSION['profile_id']!=2) && ($_SESSION['profile_id']!=3))
                            				{
                                				if($_GET['tab']=='attachment') {echo '<li class="active" >';} else {echo '<li>';} echo '
                                					<a href="./index.php?page=admin&subpage=user&action=edit&id='.$_GET['id'].'&tab=attachment">
                            			...
                            			<div class="tab-content">
                            			    <div id="attachment" class="tab-pane'; if ($_GET['tab']=='attachment' || $_GET['tab']=='') echo 'active'; echo '">
                                                ...
                                                <div class="space-4"></div>
                                                ';
                                                    $query = mysql_query('SELECT * FROM `tusers_tech` WHERE tech='.$_GET['id'].'');
                                                    while ($row=mysql_fetch_array($query))
                                                    {
                                                        //find tech name
                                                    	...
                                                    	echo '<a title="Supprimer" href="./index.php?page=admin&subpage=user&action=edit&id='.$_GET['id'].'&tab=attachment&attachmentdelete='.$row['id'].'"> <i class="icon-trash red bigger-120"></i></a>';
                                                        echo '<br />';
                                                    }
                                                    echo '
                    										...
                    										<div class="space-4"></div>';
                    											...
                    											if ($row[0]!='')
                    											{
                    												...
                    												while ($row=mysql_fetch_array($query))
                    												{
                    													...
                    													<a title=\"Supprimer cette Vue\" href=\"index.php?page=admin&subpage=user&action=edit&id=$_GET[id]&viewid=$row[id]&deleteview=1\"><i class=\"icon-trash red bigger-120\"></i></a>
                    													<br />";
                    												}
                    												echo '<br />';
                    											}
...
else if ($_GET['action']=="add" && (($_SESSION['user_id']==$_GET['id']) || ($_SESSION['profile_id']==0 || $_SESSION['profile_id']==4)))
{
	//display add form
	echo '
		<div class="col-sm-5">
...
else if (($_GET['action']=='edit') && ($_GET['id']!='') && ($_SESSION['profile_id']!=0 || $_SESSION['profile_id']!=4) && ($_GET['id']!=$_SESSION['user_id']))
{
...
else if (($_GET['action']=='add') && ($_SESSION['profile_id']!=0 || $_SESSION['profile_id']!=4) && ($_GET['id']!=$_SESSION['user_id']))
{
